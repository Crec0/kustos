<script lang='ts'>
    ...
    type EventWithTarget = Event & { currentTarget: EventTarget & HTMLInputElement };
    ...
    async function removeRemoteBlob(blobName: string) {
        ...
        console.log('remoteBlob', remoteBlob, $remoteBlobs);
        if (!remoteBlob) return;
        const blobId = remoteBlob.id;

        console.log('blobId', blobId);

        const response = await fetch(`/api/blobs/${ $page.params.id }/${ blobId }`, {
            method: 'DELETE',
        });
        if (response.ok) {
...
<Dialog bind:open={$dialogOpen}>
    <DialogTrigger>
        <Button class='w-full text-base' variant='outline'>Upload {fileType}</Button>
    </DialogTrigger>
    <DialogContent class='max-w-[60rem]'>
        <DialogHeader>
            <DrawerTitle>Upload {fileType}</DrawerTitle>
            <DrawerDescription>Upload {fileType}s you wish to add to the project</DrawerDescription>
        </DialogHeader>
        ...
        <Card class={cn('w-full p-2')}>
            <CardHeader class='p-2'>
                ...
                <input {accept} class='hidden' id={fileType} multiple name={fileType} on:change={updateFiles}
                       type='file' />
            </CardHeader>
            <CardContent
                class={cn(
                ...
                <ScrollArea class='w-full rounded h-[50vh]'>
                    {#each $localBlobMap as [ _, blob ] (blob.name)}
                        <div
                            ...
                            <span class='grow truncate rounded'> {blob.name} </span>
        ...
        <div class='flex w-full flex-col gap-2 max-w-[60rem] md:mx-auto md:flex-col'>
            <Button class='w-full text-base' on:click={uploadFiles}>Upload {fileType}s</Button>
            <Button class='w-full' on:click={() => $dialogOpen=false} variant='secondary'> Cancel</Button>
        </div>
    </DialogContent>
</Dialog>
    ...
    {#each $localBlobMap as [ _, blob ] (blob.name)}
        <div
            ...
            <Button
                class='SELECTION bg-destructive'
                variant='destructive'
                on:click={() => removeRemoteBlob(blob.name)}
            >
                <Trash2 class='h-6 w-6' />
            </Button>
        </div>
